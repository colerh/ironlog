...
function addLiftRow(container, exercise='', sets=[{}]) {
  ...
  exSel.addEventListener('change', () => {
    ...
    setRows.querySelectorAll('.set-group').forEach(g => {
      ...
      const isBWNow = g.querySelector('.bw-toggle')?.classList.contains('active');
      ...
      // Re-attach BW toggle listener
      const bwToggle = g.querySelector('.bw-toggle');
      const wtInput  = g.querySelector('.set-weight');
      if (bwToggle) bwToggle.addEventListener('click', () => {
        const on = bwToggle.classList.toggle('active');
        wtInput.style.display = on ? 'none' : '';
        if (on) wtInput.value = '';
      });
    });
  });
...
function buildWeightInput(weight, showBW) {
  ...
  const bwActive = (!weight || weight === 'BW');
  const numVal   = (weight && weight !== 'BW') ? weight : '';
  return `<div class="weight-cell-bw">
    <button type="button" class="bw-toggle${bwActive ? ' active' : ''}">BW</button>
    <input type="number" placeholder="lbs" value="${numVal}" class="set-weight" min="0" inputmode="decimal"${bwActive ? ' style="display:none"' : ''}>
  </div>`;
}
      ...
      return {
        ...
        lifts: Array.from(div.querySelectorAll('.lift-card')).map(card => ({
          ...
          sets: Array.from(card.querySelectorAll('.set-group')).map(g=>({
            weight: g.querySelector('.bw-toggle')?.classList.contains('active') ? 'BW' : g.querySelector('.set-weight').value,
